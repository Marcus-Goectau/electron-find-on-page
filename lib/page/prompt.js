...
function promptRegister() {
	...
	const matchCaseToggle = document.querySelector('#match-case');
	const wholeWordToggle = document.querySelector('#whole-word');

	matchCaseToggle.addEventListener('change', function() {
	...
	// add keyboard support for match case toggle
	matchCaseToggle.addEventListener('keydown', function(event) {
	...
	// add keyboard support for whole word toggle
	wholeWordToggle.addEventListener('keydown', function(event) {
		if (event.key === 'Enter' || event.key === ' ') {
			event.preventDefault();
			this.checked = !this.checked;
			this.dispatchEvent(new Event('change'));
